    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          -- Only register keymaps once per buffer, regardless of how many LSP clients attach.
          -- LspAttach fires once per client, so without this guard a second client attaching
          -- (e.g. eslint alongside typescript-tools) would register every keymap again,
          -- causing duplicate results in pickers like Telescope lsp_definitions.
          if vim.b[event.buf].lsp_keymaps_registered then return end
          vim.b[event.buf].lsp_keymaps_registered = true
